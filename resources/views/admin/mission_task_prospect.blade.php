@extends('layout.backend.app', ['title' => 'Prospect Review Task'])
...
<div class="container-fluid">
    ...
    <div class="card shadow border-0 mb-4" style="border-radius:1rem;">
        <div class="card-body">
            <h4 class="mb-3">Prospect Review Task</h4>
            ...
            <div class="mb-2">
                <b>Current Stage:</b>
                {{ optional($prospect->temperature)->tempName ?? '-' }}
            </div>
        </div>
    </div>

    <form method="POST" action="{{ route('missions.task.prospect.update', $task->id) }}">
        @csrf
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Prospect No</div>
                        <div class="font-weight-bold">{{ $prospect->prospect_no ?? '-' }}</div>
                    </div>

                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Source</div>
                        <div class="font-weight-bold">{{ $prospect->prospect_source ?? '-' }}</div>
                    </div>

                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Province</div>
                        <div class="font-weight-bold">{{ optional(optional($prospect->hospital)->province)->name ?? '-' }}</div>
                    </div>

                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Hospital</div>
                        <div class="font-weight-bold">{{ optional($prospect->hospital)->name ?? '-' }}</div>
                    </div>

                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Department</div>
                        <div class="font-weight-bold">{{ optional($prospect->department)->name ?? '-' }}</div>
                    </div>

                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Business Unit</div>
                        <div class="font-weight-bold">{{ optional($prospect->unit)->name ?? '-' }}</div>
                    </div>
                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Product</div>
                        <div class="font-weight-bold">{{ optional($prospect->config)->name ?? '-' }}</div>
                    </div>
                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Qty</div>
                        <div class="font-weight-bold">{{ optional($prospect)->qty ?? '-' }}</div>
                    </div>
                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">PIC</div>
                        <div class="font-weight-bold">{{ optional($prospect->personInCharge)->name ?? '-' }}</div>
                    </div>

                    <div class="col-md-6 mb-3">
                        <div class="small text-muted">Temperature</div>
                        <div class="font-weight-bold">{{ optional($prospect->temperature)->tempName ?? '-' }}</div>
                    </div>
                ...
                <div class="h6 text-uppercase mb-3">Review Update</div>

                {{-- EDIT REVIEW ONLY --}}
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label>Eta Po Date</label>
                        <input type="date" name="eta_po_date" id="eta_po_date" class="form-control">
                    </div>
                    <div class="col-md-6 mb-3">
