...
'title' => 'Approval Consumables Prospect',
'pageTitle' =>'Approval Consumables Prospect',
])
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="validationForm">
          ...
          <!-- <div class="form-group">
            <label for="etapodate">Target PO</label>
...
<script type="text/javascript">
  ...
  $('body').on("click", ".btn-validasi", function() {
    ...
    $.ajax({
      url: "{{ route('admin.consprospectvalidation', ['consumablesProspect' => ':id']) }}".replace(':id', id),
      method: "GET",
      success: function(response) {
        $("#PIC").hide();
        $("#infoinput").hide();
  ...
  function submitValid() {
    ...
    $.ajax({
      url: "{{ route('admin.consprospectvalidationupdate', ['consumablesProspect' => ':id']) }}".replace(':id', id),
      method: "PATCH",
    ...
    $.ajax({
      ...
      success: function(response) {
       ...
       // populateSelectFromDatalist('sourceedit', response.sourceoption.source, "Pilih Sumber Informasi");
        ...
        editHosPopulateSelect(sourceSelect,response.sourceoption.source,response.prospect.prospect_source,{width: '100%'})
        
        
        var provinceSelect = $("#provinceedit");

        editConfPopulateSelect(provinceSelect,response.provopt,response.prospect.province_id,{width: '100%'});
        ...
        var idcat = response.prospect.category_id;
  ...
  $(document).on('click', '.btn-vdet', function() {
         var id = $(this).attr("id");
        console.log(id);
        $.ajax({
            url: "{{ route('admin.cpvalidationdetail', ['consumablesProspect' => ':prospect']) }}".replace(':prospect', id), // Replace with your actual endpoint
            method: 'GET',
            success: function(response) {
                // Populate the modal with the response data
                var coreDataHtml = `
                    <p>${response.coredata.hospital} - ${response.coredata.department}</p>
                    <p>${response.coredata.creator} / ${response.coredata.created_at}</p>
                  
                   
                    <p>${response.coredata.unit} -  ${response.coredata.category}</p>
                  
                `;

                var showDataHtml = '';
                response.showdata.forEach(function(data, index) {

                    showDataHtml += `<hr><h5>Q${index + 1}</h5><p>${data}</p>`;
                });

                $('#productDetailsContent').html(coreDataHtml + showDataHtml);
